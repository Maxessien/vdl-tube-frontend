...
const FormatsListCard = ({ quality, size, openFormatFn = () => null }) => {
  return (
    ...
    >
      ...
      <p className="text-xl text-(--text-primary) font-bold">
        Size - {Number.isFinite(size) ? `${size / (1024 * 1024)}MB` : "Not available"}
      </p>
    </li>
  );
};

const VideoFormats = ({ formats = [], url, title, thumbnails, chapters }) => {
    ...
    <section className="px-3 py-4">
      ...
      {(!openFormat.isOpen || openFormat.id.trim().length <= 0) && (
        <ul className="space-y-4">
          {formats.map(({ height, filesize, format_id }) => (
            <FormatsListCard
              key={format_id}
              openFormatFn={() => {
